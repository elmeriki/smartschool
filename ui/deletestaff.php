<?php
include 'main_menu.php';

?>
     <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">

        <div class="container_fluid">
<div class="container_fluid">
    <!-- <h1 class="well">Delete Registration </h1> -->
    <p class="page-header"> 
    <strong>Deelete  Staff/Teacher Account</strong><br>

          The easiest solution to take your school to the next level My Smart School is the easiest and 
          complete system to manage the school better, make communication effortless and effective,
           improve student’s performances and increase the efficiency of teachers.
 </p>
	<div class="col-lg-6 well">
	<div class="row">
				<form>
					<div class="col-sm-12">
				
						</div>						
					<div class="form-group">
						<label>Teacher/Staff ID/Number</label>
						<input type="text" placeholder="Enter Teacher/Staff ID/Number.." class="form-control">
					</div>		
					<div class="form-group">
						<label>Email Address</label>
						<input type="text" placeholder="Enter Email Address Here.." class="form-control">
					</div>	
				
					<button type="button" class="btn btn-lg btn-info">Delete</button>					
					</div>
				</form> 
				</div>
	</div>
	</div>
</div> 


            </div>
          </div> 
